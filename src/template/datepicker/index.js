...
const conf = {
  ...
  renderCalendar(curYear, curMonth, curDate) {
    ...
    const { todoLabels } = this.data.datepicker || {}
  ...
  calculateDays(year, month, curDate) {
    ...
    if (this.config.noDefault) {
      ...
      selectedDay = curDate
        ...
        : this.data.datepicker.selectedDay
    }
    ...
    const tmp = { 'datepicker.days': days }
    if (curDate) {
      tmp['datepicker.selectedDay'] = selectedDay
    }
    this.setData(tmp)
  },
  /**
  ...
  tapDayItem(e) {
    ...
    let { days, selectedDay: selectedDays } = this.data.datepicker || [] // 所有选中日期
  ...
  whenMulitSelect(opts = {}) {
    ...
    this.setData({
      ...
      'datepicker.selectedDay': selectedDays
    })
  ...
  whenSingleSelect(opts = {}) {
    ...
    if (datepicker.todoLabels) {
      // 过滤所有待办日期中当月有待办事项的日期
      shouldMarkerTodoDay = datepicker.todoLabels.filter(
    ...
    this.setData({
      'datepicker.days': days,
      'datepicker.selectedDay': [currentSelected]
    })
  ...
  setTodoLabels(options = {}) {
    const { datepicker } = this.data
    if (!datepicker || !datepicker.days) {
      return warn('请等待日历初始化完成后再调用该方法')
    }
    const days = datepicker.days.slice()
    const { curYear, curMonth } = datepicker
    const {
      days: todoDays = [],
      pos = 'bottom',
      dotColor = '',
    ...
    const { todoLabels = [], todoLabelPos, todoLabelColor } = datepicker
    ...
    shouldMarkerTodoDay.concat(currentMonthTodoLabels).forEach(item => {
      ...
      if (target.showTodoLabel && item.todoText) target.todoText = item.todoText
    })
    const o = {
      'datepicker.days': days,
      'datepicker.todoLabels': uniqueTodoLabels(todoDays.concat(todoLabels))
    }
    if (!circle) {
      if (pos && pos !== todoLabelPos) o['datepicker.todoLabelPos'] = pos
      if (dotColor && dotColor !== todoLabelColor) {
        o['datepicker.todoLabelColor'] = dotColor
      }
    } else {
      o['datepicker.todoLabelCircle'] = circle
    }
    this.setData(o)
  },
  /**
  ...
  deleteTodoLabels(todos) {
    ...
    const { days, curYear, curMonth } = this.data.datepicker
    ...
    this.setData({
      'datepicker.days': days,
      'datepicker.todoLabels': todoLabels
    })
  },
  /**
   * 清空所有日期的待办事项
   */
  clearTodoLabels() {
    const { days = [] } = this.data.datepicker
    const _days = [].concat(days)
    _days.forEach(item => {
      item.showTodoLabel = false
    })
    this.setData({
      'datepicker.days': _days,
      'datepicker.todoLabels': []
    })
  },
  /**
   * 跳转至今天
  ...
  calendarTouchend(e) {
    const self = currentPage
    self.setData({
      'datepicker.leftSwipe': 0,
      'datepicker.rightSwipe': 0
    })
  },
  /**
   * 更新当前年月
  ...
  updateCurrYearAndMonth(type) {
    let { days, curYear, curMonth } = this.data.datepicker
  ...
  initSelectedDay(days) {
    ...
    const { selectedDay = [], todoLabels = [] } = this.data.datepicker
  ...
  calculateNextWeekDays() {
    ...
    let { curYear, curMonth } = this.data.datepicker
    let days = []
  ...
  switchWeek(view) {
    ...
    const { selectedDay = [], curYear, curMonth } = this.data.datepicker
   ...
   */
  disableDays(data) {
    const { disableDays = [], days } = this.data.datepicker
    ...
    this.setData({
      'datepicker.days': days,
      'datepicker.disableDays': _disableDays
    })
  },
  /**
   * 点击输入框调起日历选择器
...
export const getSelectedDay = () => {
  return currentPage && currentPage.data.datepicker.selectedDay
}
/**
 * 跳转至指定日期
 */
export const jump = (year, month, day) => {
  const self = currentPage
  const { selectedDay } = self.data.datepicker
  if (
    selectedDay &&
...
export const getTodoLabels = () => {
  return currentPage && currentPage.data.datepicker.todoLabels
}
/**
 * 切换周月视图
...
export const enableArea = (area = []) => {
  const self = currentPage
  const { enableDays = [] } = self.data.datepicker
  ...
  if (area.length === 2) {
    ...
    if (startTimestamp > endTimestamp) {
      ...
      let { days = [], selectedDay = [] } = self.data.datepicker
      const daysCopy = days.slice()
      daysCopy.forEach(item => {
        const timestamp = newDate(item.year, item.month, item.day).getTime()
        if (
      ...
      self.setData({
        'datepicker.days': daysCopy,
        'datepicker.selectedDay': selectedDay,
        'datepicker.enableArea': area,
        'datepicker.enableAreaTimestamp': [startTimestamp, endTimestamp]
      })
    }
  } else {
...
export function enableDays(days = []) {
  const self = currentPage
  const { enableArea = [], enableAreaTimestamp = [] } = self.data.datepicker
  let expectEnableDaysTimestamp = []
  if (enableArea.length) {
  ...
  let { days: allDays = [], selectedDay = [] } = self.data.datepicker
  ...
  self.setData({
    'datepicker.days': daysCopy,
    'datepicker.selectedDay': selectedDay,
    'datepicker.enableDays': days,
    'datepicker.enableDaysTimestamp': expectEnableDaysTimestamp
  })
}

export function showDatepicker(date) {
  conf.showDatepicker(date)
}

export function closeDatepicker() {
  conf.closeDatePicker()
}
